...
Este plugin foi desenvolvido para o [Borsalinux-fr (anteriormente
Fedora-Fr)](https://www.borsalinux-fr.org). Ele gera um cartão de membro em PDF
que pode estar vazio (para ser usado durante eventos, etc.) ou preenchido com
informações do membro (que pode ser cadastrado mediante verificação de
associação, por exemplo).

**Note**: A partir da versão 0.8.3 do Galette, é fornecido um cartão de membro
em PDF baseado em modelos PDF. A instalação do Fullcard 1.6 ou posterior
substituirá este modelo.
